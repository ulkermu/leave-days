...
const Footer = () => {
  ...
  return (
    <>
      <Divider className='max-w-[1280px] w-full mx-5' />
      <footer className='flex max-w-[1280px] w-full justify-around p-5 max-[640px]:flex-col max-[640px]:gap-5'>
        <div className='max-w-md flex flex-col gap-3 max-[640px]:max-w-full'>
          ...
          <p>Creating a system where we can input the leave days used within the company and add or remove employees.</p>
        </div>
            ...
            {links.map((link, key) => (
              <Link key={key} href={link.location} target='_blank'>
                <IconButton className='dark:hover:bg-slate-500 rounded-full ease-out duration-150'>
                  {link.name === "twitter" && <TwitterIcon sx={{ color: 'var(--text)' }} />}
                  {link.name === "github" && <GitHubIcon sx={{ color: 'var(--text)' }} />}
                  {link.name === "linkedin" && <LinkedInIcon sx={{ color: 'var(--text)' }} />}
                </IconButton>
              </Link>
            ))}
          </div>
